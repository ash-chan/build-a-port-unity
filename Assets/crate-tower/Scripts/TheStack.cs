...

{
    ...
    private void MoveTile()
    {
		...
		if (actualScore >= 150)
			tileSpeed = 3.5f;
		else if (actualScore >= 100 && actualScore < 150)
			tileSpeed = 3.0f;
		else if (actualScore >= 50 && actualScore < 100)
			tileSpeed = 2.5f;
		else
			tileSpeed = 2.0f;
    ...
    private bool PlaceTile()
    {
        ...
        if (isMovingOnX)
        {
            ...
            else
            {
				...
				GetComponent<AudioSource> ().PlayOneShot (cratePerfect);
            }
        }
        else
        {
    ...
    private void EndGame()
    {
        ...
        if (Input.GetMouseButtonDown(0)) {
            ...
            moneyEarnedText.text = "$ " + (actualScore * 12).ToString();
            Money.currentMoney += actualScore * 12;
        }
    }
